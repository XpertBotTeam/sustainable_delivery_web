    ...
    <div class="bg-white rounded-[42px] shadow-[0_4px_10px_1px_rgba(12,12,12,0.37)] p-5 ">
        ...
        <div class="min-h-full flex flex-col justify-between flex-1">
            <div class="flex gap-5 items-center">
                ...
                <div class="Roboto text-[18px] text-[#929292] group-hover:text-white duration-[300ms] cursor-pointer">{{  (props.order && props.order.user && props.order.user.name) ? props.order.user.name : ''  }}</div>
            </div>
            <div class="flex gap-5 items-center">
                <div class="DarkerGrotesque text-[24px]">Address</div>
                <div class="Roboto text-[18px] text-[#929292] group-hover:text-white duration-[300ms] cursor-pointer">{{  address }}</div>
            </div>
            <div class="flex gap-5 items-center">
                <div class="DarkerGrotesque text-[24px]">Order Date</div>
                <div class="Roboto text-[18px] text-[#929292] group-hover:text-white duration-[300ms] cursor-pointer">{{props.order.orderDate ?new Date(props.order.orderDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric', timeZoneName: 'short' }) : ''}}</div>
            </div>
            <div class="flex gap-5 items-center">
                <div class="DarkerGrotesque text-[24px]">Price</div>
                <div class="Roboto text-[18px] text-[#929292] group-hover:text-white duration-[300ms] cursor-pointer">{{props.order.total}}$</div>
            </div>
            <div class="flex gap-5 items-center">
                <div class="DarkerGrotesque text-[24px]">Delivery Guy</div>
                <div class="Roboto text-[18px] text-[#929292] group-hover:text-white duration-[300ms] cursor-pointer">{{(props.order && props.order.deliveryGuy && props.order.deliveryGuy.name) ?
            ...
            <div class="Roboto text-[18px] text-[#929292] group-hover:text-white duration-[300ms] cursor-pointer">Click to change the order status (pending,preparing,prepared)</div>
        </div>
        <img class="max-w-[176px] h-fit" :src="packageImage" />
       </div>
       <div>
        <div class="DarkerGrotesque text-[24px]">Order Details</div>
        <div class="Roboto text-[18px] text-[#929292] group-hover:text-white duration-[300ms] cursor-pointer mb-5">{{ props.order.companyOrders[0].status }}</div>
        ...
        <div class="flex justify-between max-w-[400px]">
            ...
            <div class="Roboto text-[18px] text-[#929292] group-hover:text-white duration-[300ms] cursor-pointer mb-5">{{ totalPrice}} $</div>
        </div>
        <div class="flex justify-between  max-w-[400px]">
            <div class="Montserrat text-[18px]  group-hover:text-white duration-[300ms] cursor-pointer ">PAYMENT METHOD</div>
            <div class="Roboto text-[18px] text-[#929292] group-hover:text-white duration-[300ms] cursor-pointer mb-5">Cash On Delivery</div>
        </div>
