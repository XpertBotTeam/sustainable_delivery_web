...
<template>
    <div class= "mb-10 md:mb-20">
        <div class="w-full" v-if="props.CompanyProduct.bannerImage">
        ...
        <div class="w-[85%] mx-auto">
            <div class="w-full flex justify-between mb-10">
                ...
                <div class="DarkerGrotesque w-fit">
                    <div class="text-[16px] viewall">view all</div>
                    <div class="w-fit ml-auto"><SvgIcon class="" iconName="More"></SvgIcon></div>
                </div>
            </div>

            <div v-if=" props.CompanyProduct.products.length > 0" class="flex max-md:overflow-scroll md:grid grid-cols-3 gap-5">
                <ProductsCards  v-for="product in props.CompanyProduct.products" :Product="product" :key="product.productId"></ProductsCards>
            </div>
        </div>
    </div>
</template>